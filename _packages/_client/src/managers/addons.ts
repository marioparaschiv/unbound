import type { Addon, AddonManifest, AddonResolveable } from '@unbound-app/types/managers';
import { ManagerEntity, type ManagerKind, ManagerType, Regex } from '~/lib/constants';
...
class Addons<T extends Addon> extends Base<T> {
	...
	load(bundle: string, manifest: AddonManifest): T {
	...
	async install(url: string): Promise<boolean> {
		try {
			...
			try {
				...
				this.validateManifest(manifest as AddonManifest);
				manifest.url = url;
			} catch (error) {
	...
	async delete(entity: AddonResolveable) {
		const addon = this.resolve(entity);
		if (!addon) return;
	...
	resolve(id: AddonResolveable): T | void {
