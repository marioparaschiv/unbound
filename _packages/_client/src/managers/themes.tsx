import type { AddonManifest, ThemeEntity, AddonResolveable } from '@unbound-app/types/managers';
...
class Themes extends Addons<ThemeEntity> {
	...
	override load(bundle: string, manifest: AddonManifest): ThemeEntity {
	...
	override enable(entity: AddonResolveable): void {
	...
	override disable(entity: AddonResolveable): void {
		const addon = this.resolve(entity);
		if (!addon) return;
	...
	registerValues(theme: ThemeEntity) {
	...
	patchColors() {
		...
		this.patcher.instead(InternalResolver, 'resolveSemanticColor', (self, args: [theme: string, ref: { [key: symbol]: string; }], orig) => {
			...
			if (!symbol) return orig.call(this, instance?.type ?? 'darker', ref);

			const key = ref[symbol];
			if (!key) return orig.call(this, instance?.type ?? 'darker', ref);
