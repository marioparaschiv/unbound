 ...
 */
function forceRender<T extends (...args: any[]) => JSX.Element>(component: T, context?: any): (...args: Parameters<T>) => ReturnType<T> | null {
	return (...args) => {
		...
		try {
			res.rendered = (context ? component.call(context, args) : component(...args)) as ReturnType<T>;
		} catch (error) {
			res.error = error;
		}
